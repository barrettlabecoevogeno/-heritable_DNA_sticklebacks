library(methylKit)
load("./meth.RData")
# Subset KL_F2 samples
f2_subset=read.csv("./stickleback_metadata.csv")
f2_subset=f2_subset[f2_subset$Generation=="F2" & f2_subset$Origin=="KL", ]
...
library(dplyr)
snps_ind=read.table("./kl_f2.012.indv")
...
meth=reorganize(meth.all,
                ...
                treatment = as.numeric(f2_subset[,3])) # KL is 3
...
write.table(cpg, "./cpg.kl.f2.txt", row.names = F, sep = "\t", quote = F)
...
snps=read.table("./kl_f2.012", row.names = 1) # 351 SNPs

# exclude snps on sex chromosome, LG19
snps_pos=read.table("./kl_f2.012.pos")
...
write.table(snps, "./snps.kl.f2.txt", row.names = F, sep = "\t", quote = F) # 2692 SNPs

#### Last make a matrix of covariate(s)-------
covariate=read.csv("./stickleback_metadata.csv")
covariate=covariate[covariate$Generation=="F2" & covariate$Origin=="KL",]
...
write.table(covariate, "./covariate.kl.f2.txt", col.names = F, sep = "\t", quote = F)

library(MatrixEQTL)
base.dir = "./"
useModel = modelLINEAR
SNP_file_name = paste(base.dir, "snps.kl.f2.txt", sep="")
meth_file_name = paste(base.dir, "cpg.kl.f2.txt", sep="")
covariates_file_name = paste(base.dir, "covariate.kl.f2.txt", sep="") 
output_file_name = paste(base.dir, "output.kl.f2.txt", sep="") 
...
snps$LoadFile(SNP_file_name)    # 330 SNPs
...
meth.qtl$LoadFile(meth_file_name)   # 27039 CpGs
...
output=read.table("./output.kl.f2.txt", sep = "\t", skip = 1)
...
nrow(output.sig) # 531 sig mQTL

write.table(output.sig, "sig.meqtl_kl_f2.txt", sep = "\t", quote = F)

# Check number of unique SNPs and CpGs
output.sig$SNP=as.character(output.sig$SNP) 
output.sig$CpG=as.character(output.sig$CpG)

length(unique(output.sig$SNP)) # 201 SNPS
length(unique(output.sig$CpG)) # 72 CpGs, 72/27039=0.00266282
...
nrow(output.sig[output.sig$cis_trans=="cis",]) # 4 cis
nrow(output.sig[output.sig$cis_trans=="trans",]) # 527 trans
...
GTest(x=observed,
      p=expected,
      correct="none") 
# G = 689.04, X-squared df = 1, p-value < 2.2e-16
...
nrow(unique(overlaps_eqtl_mqtl[,1:3])) # 25 overlap, 201-25=176 not overlap
...
nrow(unique(overlaps_eqtl_input_snp[,1:3])) # 34 overlap, 330-34=296 not overlap

# test whether meqtl overlap with eqtl is significant
# G-test
observed = c(25, 176)
expected = c(34/330, 296/330)
library(DescTools)
GTest(x=observed,
      p=expected,
      correct="none") 
# G = 0.93628, X-squared df = 1, p-value = 0.3332
...
# Genes in meqtl
pool_gene_pos=read.table("./kl_f2.012.pos")
...
for (i in 1:length(ontology)) {
  ...
  write.csv(allRes, paste("kl_mqtl_genes",ontology[i],"csv",sep="."))
} 
...
nrow(unique_mqtl_divergence_overlap) # 2 unique mQTL overlap with divergence region
...
nrow(unique_mqtl_cis_trans[unique_mqtl_cis_trans$cis_trans=="cis",]) # 0
nrow(unique_mqtl_cis_trans[unique_mqtl_cis_trans$cis_trans=="trans",]) # 3

# G-test
observed = c(0, 3)
expected = c(4/531, 527/531)
library(DescTools)
GTest(x=observed,
      p=expected,
      correct="none") 
# G = 0.045369, X-squared df = 1, p-value = 0.8313

## effect size of the 3 meQTLs
...
# 2 mQTL overlapped 2 genes
write.csv(metadata_unique_mqtl_divergence_overlap_gene, "./kl_mqtl_divergence.csv", row.names = F, quote = F)
...
nrow(unique(overlaps_dmcs_parental_cpg_in_mqtl[,1:3]))
# 2 cpg overlapping with dmcs in parental generation
...
nrow(snp_underlying_dmcs_in_parental) # 9
nrow(snp_underlying_dmcs_in_parental[snp_underlying_dmcs_in_parental$cis_trans=="cis",]) 
# 0
nrow(snp_underlying_dmcs_in_parental[snp_underlying_dmcs_in_parental$cis_trans=="trans",]) 
# 9

# check genes associated with the 9 SNPs
...
# merge all information for the 9 SNPs (locations, names, go id, go terms, etc...)
...
length(unique(special_cpg_associated_snp$ensembl_gene_id)) # 4 genes annotated with the 9 SNPs
# for export
...
write.csv(special_cpg_associated_snp.1, "./kl_snps_underlying_special_dmc_full_info.csv", row.names = F)
...
# what is the gene associated with the two special dmc, because only one dmc is associated with sig meQTL, so we just use the first row as an presentation
special_dmc_loc=overlaps_dmcs_parental_cpg_in_mqtl[c(1,5),1:3]
special_dmc_granges=as(special_dmc_loc, "GRanges")
gene_special_dmc=annotatePeakInBatch(special_dmc_granges, AnnotationData = genes, output = "nearestLocation")
gene_special_dmc=as.data.frame(gene_special_dmc)

# full information of this gene associated with the two cpg
gene_special_dmc_info=getBM(filters = "ensembl_gene_id", 
                            attributes = c("ensembl_gene_id", "external_gene_name","go_id", "name_1006", "description"), 
                            values = gene_special_dmc$feature, 
                            mart = mart)

write.csv(snp_underlying_dmcs_in_parental, "./kl_snp_underlying_dmcs_in_parental.csv")
write.csv(gene_special_dmc_info, "./kl_gene_special_dmc_info.csv")
